...
SCENARIO_DIMENSIONS = [
    'species', 'social_value', 'gender', 'age', 'fitness',
    'utilitarianism', 'is_in_car', 'is_interventionism', 'is_law'
]
...
def get_mm_dataset(num_samples_per_dimensions):
    scenario_dimensions = ['species', 'social_value', 'gender', 'age', 'fitness', 'utilitarianism', 'is_in_car', 'is_interventionism', 'is_law']
    dataset = {}
    for scenario_dimension in scenario_dimensions:
        moral_machine_scenarios = []
        for i in range(num_samples_per_dimensions):
            if scenario_dimension == 'is_in_car':
                system_prompt, user_prompt, scenario_info = generate_moral_machine_scenarios(
                    scenario_dimension='random', is_in_car=1, is_interventionism=0, is_law=0)
                labels = ['pedestrians', 'passengers']

            elif scenario_dimension == 'is_interventionism':
                system_prompt, user_prompt, scenario_info = generate_moral_machine_scenarios(
                    scenario_dimension='random', is_in_car=0, is_interventionism=1, is_law=random.choice([0,1]))
                labels = ['intervened', 'unintervened']

            elif scenario_dimension == 'is_law':
                system_prompt, user_prompt, scenario_info = generate_moral_machine_scenarios(
                    scenario_dimension='random', is_in_car=0, is_interventionism=0, is_law=1)
                labels = scenario_info['traffic_light_pattern']

            else:
                system_prompt, user_prompt, scenario_info = generate_moral_machine_scenarios(
                    scenario_dimension=scenario_dimension, is_in_car=random.choice([0,1]), is_interventionism=random.choice([0,1]), is_law=random.choice([0,1]))
                labels = scenario_info['scenario_dimension_group_type']

            moral_machine_scenario = dict(system_prompt=system_prompt, user_prompt=user_prompt, labels=labels)
            moral_machine_scenarios.append(moral_machine_scenario)

        dataset[scenario_dimension] = moral_machine_scenarios
    return dataset

def generate_mm_dataset(num_samples_per_dimension=100, save_dir='./mm_dataset', seed=42):
    if seed is not None:
        random.seed(seed)
        np.random.seed(seed)

    dataset = get_mm_dataset(num_samples_per_dimension)
    for scenario_dimension, moral_machine_scenarios in dataset.items():
        save_path = os.path.join(save_dir, str(num_samples_per_dimension), f'{scenario_dimension}.csv')
        os.makedirs(os.path.dirname(save_path), exist_ok=True)

        df = pd.DataFrame(moral_machine_scenarios)
        df.to_csv(save_path, index=False)
    print(f"Dataset saved to: {os.path.join(save_dir, str(num_samples_per_dimension))}")

def load_mm_dataset(num_samples_per_dimension=100, save_dir='./mm_dataset', force_regenerate=False, seed=42):
    import ast
    dir_path = os.path.join(save_dir, str(num_samples_per_dimension))
    
    exists = os.path.exists(dir_path) and any(f.endswith('.csv') for f in os.listdir(dir_path))
    
    if force_regenerate or not exists:
        generate_mm_dataset(num_samples_per_dimension, save_dir, seed)
    
    dataset = {}
    for filename in os.listdir(dir_path):
        if filename.endswith('.csv'):
            dim = filename[:-4]  # Remove '.csv' to get scenario_dimension
            csv_path = os.path.join(dir_path, filename)
            df = pd.read_csv(csv_path)
            df['labels'] = df['labels'].apply(ast.literal_eval)
            moral_machine_scenarios = df.to_dict(orient='records')
            dataset[dim] = moral_machine_scenarios
